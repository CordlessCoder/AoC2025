...
impl<T, const CAP: usize> ArrayVec<T, CAP> {
    ...
    pub fn try_insert(&mut self, index: usize, value: T) -> Result<(), T> {
        if index >= self.capacity() || self.is_full() {
            return Err(value);
        }
        if index == self.len() {
            return self.try_push(value);
        }
        let moved = unsafe { self.get_unchecked_mut(index..) };
        moved.rotate_right(1);
        let last = core::mem::replace(&mut moved[0], value);
        // SAFETY: We ensured we have the space for this with the is_full check
        unsafe { self.try_push(last).unwrap_unchecked() };
        Ok(())
    }
    pub fn insert(&mut self, index: usize, value: T) {
        _ = self.try_insert(index, value)
    }
...
impl<T, const CAP: usize> ArrayVecIter<T, CAP> {
    pub fn remainder(&self) -> &[T] {
        unsafe { std::mem::transmute(&self.arr[self.idx..self.len]) }
    }
}
...
impl<const CAP: usize> Write for ArrayVec<u8, CAP> {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        let len = self.spare_capacity();
        self.extend(buf.iter().copied());
        let len = len - self.spare_capacity();
        Ok(len)
    }
